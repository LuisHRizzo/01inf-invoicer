...
interface ServiceModalProps {
    onSave: (service: { description: string; price: number; category: ServiceCategory }) => void;
    onClose: () => void;
}

const ServiceModal: React.FC<ServiceModalProps> = ({ onSave, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
            <div className="relative mx-auto p-8 border w-full max-w-md shadow-lg rounded-md bg-white">
                <h3 className="text-xl font-semibold text-gray-900 mb-6">Añadir producto o servicio</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="description" className="block text-sm font-medium text-gray-700">Nombre / descripción</label>
                        <input
                            type="text"
                            id="description"
                            name="description"
                    ...
                    <div>
                        <label htmlFor="category" className="block text-sm font-medium text-gray-700">Tipo</label>
                        <select
                            id="category"
                            name="category"
